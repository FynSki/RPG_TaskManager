...
interface LoginProps {
    ...
    onClose?: () => void; // Funkcja do zamknięcia (powrót do App)
}

export function Login({ onSwitchToRegister, onSkip, onClose }: LoginProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) {
                ...
                // SUCCESS! Auto-redirect po zalogowaniu
                if (onClose) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
            <div className="bg-slate-800 rounded-2xl shadow-2xl p-8 w-full max-w-md border border-slate-700 relative">

                {/* Back Button - podobnie jak w AboutPage */}
                {onClose && (
                    <button
                        onClick={onClose}
